...
LveDescriptorPool::Builder &LveDescriptorPool::Builder::setMaxSets(
    uint32_t count) {
   maxSets = count;
   return *this;
}

std::unique_ptr<LveDescriptorPool> LveDescriptorPool::Builder::build()
    const {
   return std::make_unique<LveDescriptorPool>(lveDevice, maxSets,
                                              poolFlags, poolSizes);
}
...
bool LveDescriptorPool::allocateDescriptor(
   ...
   if (vkAllocateDescriptorSets(lveDevice.device(), &allocInfo,
...
void LveDescriptorWriter::overwrite(VkDescriptorSet &set) {
   ...
   vkUpdateDescriptorSets(pool.lveDevice.device(), writes.size(),
