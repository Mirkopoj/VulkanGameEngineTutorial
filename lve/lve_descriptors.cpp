...
LveDescriptorSetLayout::Builder::build() const {
...
LveDescriptorSetLayout::~LveDescriptorSetLayout() {
  vkDestroyDescriptorSetLayout(lveDevice.device(), descriptorSetLayout,
                               nullptr);
}
...
LveDescriptorPool::Builder::addPoolSize(VkDescriptorType descriptorType,
                                        uint32_t count) {
...
LveDescriptorPool::Builder &
LveDescriptorPool::Builder::setPoolFlags(VkDescriptorPoolCreateFlags flags) {
  poolFlags = flags;
  return *this;
}
LveDescriptorPool::Builder &
LveDescriptorPool::Builder::setMaxSets(uint32_t count) {
  maxSets = count;
  return *this;
}

std::unique_ptr<LveDescriptorPool> LveDescriptorPool::Builder::build() const {
  return std::make_unique<LveDescriptorPool>(lveDevice, maxSets, poolFlags,
                                             poolSizes);
}
...
LveDescriptorPool::LveDescriptorPool(
    LveDevice &lveDevice, uint32_t maxSets,
...
bool LveDescriptorPool::allocateDescriptor(
  ...
  if (vkAllocateDescriptorSets(lveDevice.device(), &allocInfo, &descriptor) !=
      VK_SUCCESS) {
    return false;
  }
  return true;
...
void LveDescriptorPool::freeDescriptors(
...
LveDescriptorWriter::LveDescriptorWriter(LveDescriptorSetLayout &setLayout,
...
void LveDescriptorWriter::overwrite(VkDescriptorSet &set) {
  ...
  vkUpdateDescriptorSets(pool.lveDevice.device(), writes.size(), writes.data(),
                         0, nullptr);
}
