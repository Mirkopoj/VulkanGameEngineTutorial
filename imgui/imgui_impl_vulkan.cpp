...
// This needs to be used along with a Platform Backend (e.g. GLFW, SDL,
// Win32, custom..)

// Implemented features:
//  [!] Renderer: User texture binding. Use 'VkDescriptorSet' as
//  ImTextureID. Read the FAQ about ImTextureID! See
//  https://github.com/ocornut/imgui/pull/914 for discussions. [X]
//  Renderer: Large meshes support (64k+ vertices) with 16-bit indices.

// Important: on 32-bit systems, user texture binding is only supported if
// your imconfig file has '#define ImTextureID ImU64'. This is because we
// need ImTextureID to carry a 64-bit value and by default ImTextureID is
// defined as void*. To build this on 32-bit systems and support texture
// changes:
// - [Solution 1] IDE/msbuild: in "Properties/C++/Preprocessor Definitions"
// add 'ImTextureID=ImU64' (this is what we do in our .vcxproj files)
...
// The aim of imgui_impl_vulkan.h/.cpp is to be usable in your engine
// without any modification. IF YOU FEEL YOU NEED TO MAKE ANY CHANGE TO
// THIS CODE, please share them and your feedback at
...
// - Common ImGui_ImplVulkan_XXX functions and structures are used to
// interface with imgui_impl_vulkan.cpp/.h.
//   You will use those if you want to use this rendering backend in your
//   engine/app.
// - Helper ImGui_ImplVulkanH_XXX functions and structures are only used by
// this example (main.cpp) and by
//   the backend itself (imgui_impl_vulkan.cpp), but should PROBABLY NOT be
...
// details)
//  2023-07-04: Vulkan: Added optional support for
//  VK_KHR_dynamic_rendering. User needs to set
//  init_info->UseDynamicRendering = true and
//  init_info->ColorAttachmentFormat. 2023-01-02: Vulkan: Fixed sampler
//  passed to ImGui_ImplVulkan_AddTexture() not being honored + removed a
//  bunch of duplicate code. 2022-10-11: Using 'nullptr' instead of 'NULL'
//  as per our switch to C++11. 2022-10-04: Vulkan: Added experimental
//  ImGui_ImplVulkan_RemoveTexture() for api symetry. (#914, #5738).
//  2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet.
//  User need to call ImGui_ImplVulkan_AddTexture(). Building for 32-bit
//  targets requires '#define ImTextureID ImU64'. (#914). 2021-10-15:
//  Vulkan: Call vkCmdSetScissor() at the end of render a full-viewport to
//  reduce likehood of issues with people using VK_DYNAMIC_STATE_SCISSOR in
//  their app without calling vkCmdSetScissor() explicitly every frame.
...
//  bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory validation error when
//  buffer sizes are not multiple of
//  VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change
//  blending equation to preserve alpha in output buffer. 2021-01-27:
//  Vulkan: Added support for custom function load and
//  IMGUI_IMPL_VULKAN_NO_PROTOTYPES by using
//  ImGui_ImplVulkan_LoadFunctions(). 2020-11-11: Vulkan: Added support for
//  specifying which subpass to reference during VkPipeline creation.
//  2020-09-07: Vulkan: Added VkPipeline parameter to
//  ImGui_ImplVulkan_RenderDrawData (default to one passed to
//  ImGui_ImplVulkan_Init). 2020-05-04: Vulkan: Fixed crash if initial
//  frame has no vertices. 2020-04-26: Vulkan: Fixed edge case where render
//  callbacks wouldn't be called if the ImDrawData didn't have vertices.
//  2019-08-01: Vulkan: Added support for specifying multisample count. Set
//  ImGui_ImplVulkan_InitInfo::MSAASamples to one of the
//  VkSampleCountFlagBits values to use, default is non-multisampled as
//  before. 2019-05-29: Vulkan: Added support for large mesh (64K+
//  vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag.
//  2019-04-30: Vulkan: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state.
//  2019-04-04: *BREAKING CHANGE*: Vulkan: Added ImageCount/MinImageCount
...
//  ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: Vulkan: Added
//  VkInstance argument to ImGui_ImplVulkanH_CreateWindow() optional
//  helper. 2019-04-04: Vulkan: Avoid passing negative coordinates to
//  vkCmdSetScissor, which debug validation layers do not like. 2019-04-01:
//  Vulkan: Support for 32-bit index buffer (#define ImDrawIdx unsigned
//  int). 2019-02-16: Vulkan: Viewport and clipping rectangles correctly
//  using draw_data->FramebufferScale to allow retina display. 2018-11-30:
//  Misc: Setting up io.BackendRendererName so it can be displayed in the
//  About Window. 2018-08-25: Vulkan: Fixed mishandled
//  VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22: Inverted
//  the parameters to ImGui_ImplVulkan_RenderDrawData() to be consistent
//  with other backends. 2018-06-08: Misc: Extracted
//  imgui_impl_vulkan.cpp/.h away from the old combined GLFW+Vulkan
//  example. 2018-06-08: Vulkan: Use draw_data->DisplayPos and
//  draw_data->DisplaySize to setup projection matrix and clipping
//  rectangle. 2018-03-03: Vulkan: Various refactor, created a couple of
//  ImGui_ImplVulkanH_XXX helper that the example can use and that viewport
//  support will use. 2018-03-01: Vulkan: Renamed
//  ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and fields to
//  match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the
//  io.RenderDrawListsFn callback, ImGui_ImplVulkan_Render() calls
//  ImGui_ImplVulkan_RenderDrawData() itself. 2018-02-06: Misc: Removed
//  call to ImGui::Shutdown() which is not available from 1.60 WIP, user
//  needs to call CreateContext/DestroyContext themselves. 2017-05-15:
//  Vulkan: Fix scissor offset being negative. Fix new Vulkan validation
//  warnings. Set required depth member for buffer image copy. 2016-11-13:
//  Vulkan: Fix validation layer warnings and errors and redeclare
//  gl_PerVertex. 2016-10-18: Vulkan: Add location decorators & change to
//  use structs as in/out in glsl, update embedded spv (produced with
//  glslangValidator -x). Null the released resources. 2016-08-27: Vulkan:
...
void ImGui_ImplVulkanH_DestroyFrame(
    VkDevice device, ImGui_ImplVulkanH_Frame* fd,
...
// (see description of IMGUI_IMPL_VULKAN_NO_PROTOTYPES in
// imgui_impl_vulkan.h
...
static PFN_vkCmdBeginRenderingKHR
...
// FIXME: multi-context support is not tested and probably dysfunctional in
// this backend.
static ImGui_ImplVulkan_Data* ImGui_ImplVulkan_GetBackendData() {
   return ImGui::GetCurrentContext()
              ? (ImGui_ImplVulkan_Data*)ImGui::GetIO()
...
static void ImGui_ImplVulkan_SetupRenderState(
    ImDrawData* draw_data, VkPipeline pipeline,
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data,
   ...
   ImGui_ImplVulkanH_WindowRenderBuffers* wrb =
   ...
   if (draw_data->TotalVtxCount > 0) {
      ...
      VkResult err =
          vkMapMemory(v->Device, rb->VertexBufferMemory, 0,
                      rb->VertexBufferSize, 0, (void**)(&vtx_dst));
      check_vk_result(err);
      err = vkMapMemory(v->Device, rb->IndexBufferMemory, 0,
                        rb->IndexBufferSize, 0, (void**)(&idx_dst));
      check_vk_result(err);
      ...
      for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
         ...
         if (pcmd->UserCallback != nullptr) {
            ...
            if (sizeof(ImTextureID) < sizeof(ImU64)) {
               // We don't support texture switches if ImTextureID hasn't
               // been redefined to be 64-bit. Do a flaky check that other
               // textures haven't been used.
               IM_ASSERT(pcmd->TextureId ==
                         (ImTextureID)bd->FontDescriptorSet);
   ...
   // Note: at this point both vkCmdSetViewport() and vkCmdSetScissor()
   ...
   // VK_DYNAMIC_STATE_VIEWPORT or VK_DYNAMIC_STATE_SCISSOR you are
   // responsible for setting the values before rendering. In theory we
   // should aim to backup/restore those values but I am not sure this is
   // possible. We perform a call to vkCmdSetScissor() to set back a full
   // viewport which is likely to fix things for 99% users but technically
   // this is not perfect. (See github #4644)
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer) {
   ...
   {
      char* map = nullptr;
      err = vkMapMemory(v->Device, bd->UploadBufferMemory, 0, upload_size,
                        0, (void**)(&map));
      check_vk_result(err);
   ...
   // Copy to Image:
   {
      ...
      vkCmdCopyBufferToImage(
          command_buffer, bd->UploadBuffer, bd->FontImage,
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
   ...
   if (!bd->PipelineLayout) {
      // Constants: we are using 'vec2 offset' and 'vec2 scale' instead of
      // a full 3d projection matrix
   ...
   ImGui_ImplVulkan_CreatePipeline(
       v->Device, v->Allocator, v->PipelineCache, bd->RenderPass,
...
bool ImGui_ImplVulkan_LoadFunctions(
   ...
   //      ImGui_ImplVulkan_LoadFunctions([](const char* function_name,
   //      void*) { return vkGetInstanceProcAddr(your_vk_isntance,
   //      function_name); });
   // But this would be equivalent to not setting VK_NO_PROTOTYPES.
#ifdef VK_NO_PROTOTYPES
#define IMGUI_VULKAN_FUNC_LOAD(func)                                    \
   func =                                                               \
       reinterpret_cast<decltype(func)>(loader_func(#func, user_data)); \
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info,
   ...
   if (info->UseDynamicRendering) {
      ...
      ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR =
          reinterpret_cast<PFN_vkCmdEndRenderingKHR>(vkGetInstanceProcAddr(
              info->Instance, "vkCmdEndRenderingKHR"));
#endif
      IM_ASSERT(ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR != nullptr);
      IM_ASSERT(ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR != nullptr);
#else
      IM_ASSERT(0 &&
                "Can't use dynamic rendering when neither VK_VERSION_1_3 "
                "or VK_KHR_dynamic_rendering is defined.");
#endif
   }
...
// Generally we avoid exposing any kind of superfluous high-level helpers
// in the backends, but it is too much code to duplicate everywhere so we
// exceptionally expose them.
//
// Your engine/app will likely _already_ have code to setup all that stuff
// (swap chain, render pass, frame buffers, etc.). You may read this code
// to learn about Vulkan, but it is recommended you use you own custom
// tailored code to do equivalent work. (The ImGui_ImplVulkanH_XXX
// functions do not interact with any of the state used by the regular
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(
   ...
   // VK_IMAGE_CREATE_MUTABLE_BIT was set at image creation Assuming that
   // the default behavior is without setting this bit, there is no need
   // for separate Swapchain image and image view format Additionally
   // several new color spaces were introduced with Vulkan Spec v1.0.40,
   // hence we must make sure that a format with the mostly available color
   // space, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR, is found and used.
   ...
   if (avail_count == 1) {
      ...
      for (int request_i = 0; request_i < request_formats_count;
           request_i++)
      ...
      // If none of the requested image formats could be found, use the
      // first available
      return avail_format[0];
   }
}
...
void ImGui_ImplVulkanH_CreateWindowCommandBuffers(
   ...
   for (uint32_t i = 0; i < wd->ImageCount; i++) {
      ...
      ImGui_ImplVulkanH_FrameSemaphores* fsd = &wd->FrameSemaphores[i];
      {
         ...
         err = vkCreateCommandPool(device, &info, allocator,
                                   &fd->CommandPool);
         check_vk_result(err);
      }
      {
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(
   ...
   // preserve the old swapchain to create the new one. Destroy old
   // Framebuffer
   for (uint32_t i = 0; i < wd->ImageCount; i++) {
   ...
   // If min image count was not specified, request different count of
   // images dependent on selected present mode
   if (min_image_count == 0)
      min_image_count = ImGui_ImplVulkanH_GetMinImageCountFromPresentMode(
          wd->PresentMode);
   ...
   if (wd->UseDynamicRendering == false) {
      ...
      attachment.loadOp = wd->ClearEnable
      ...
      // examples' main.cpp, but secondary viewport in multi-viewport mode
      // may want to create one with:
   ...
   // Create The Image Views
   {
      ...
      VkImageSubresourceRange image_range = {VK_IMAGE_ASPECT_COLOR_BIT, 0,
                                             1, 0, 1};
...
void ImGui_ImplVulkanH_DestroyWindow(
    ...
    const VkAllocationCallbacks* allocator) {
   vkDeviceWaitIdle(
       device);  // FIXME: We could wait on the Queue if we had the queue
                 // in wd-> (otherwise VulkanH functions can't use globals)
